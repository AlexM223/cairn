<script lang="ts">
	...
	// each key still exists and still derives this vault. Trezor/Ledger keys are
	// re-read live over USB/Connect and compared server-side; ColdCard/QR/file
	// keys get a guided manual check against the vault's receive address.
	...
	async function postVerified(body: Record<string, unknown>): Promise<Record<string, unknown>> {
		const res = await fetch(`/api/vaults/${vaultId}/keys/${keyInfo.id}/verified`, {
			method: 'POST',
	...
	async function runDeviceCheck() {
		...
		try {
			...
			if (keyInfo.deviceType === 'trezor') {
				...
				reading = await mod.readVaultKeyFromTrezor(scriptType, account);
			} else {
				const mod = await import('$lib/hw/ledger');
				reading = await mod.readVaultKeyFromLedger(scriptType, account);
			}
...
<div class="khr" class:expanded>
	...
	{#if expanded}
		<div class="khr-panel fade-in">
			{#if isDeviceCheck}
				<p class="khr-copy">
					Cairn asks the {deviceLabel} for its public key and compares it with what this vault
					stores. Nothing is signed, and nothing secret ever leaves the device — a match proves
					it still holds the exact key this vault expects.
				</p>
			...
			{:else}
				<p class="khr-copy">
					Cairn can't talk to this key directly, so verify it by hand against the vault's current
					receive address:
				</p>
				{#if receiveAddress}
				...
				{:else}
					<p class="khr-copy khr-warn-text">
						The receive address isn't available right now (the vault scan failed) — retry the
						scan first.
					</p>
				{/if}
				<p class="khr-copy">
					{#if keyInfo.deviceType === 'coldcard'}
						On the ColdCard: <strong>Settings → Multisig Wallets → (this vault) → Address
						Explorer</strong> and confirm this address appears. If the vault isn't listed,
						re-import the registration file from the backup card above.
					{:else if keyInfo.deviceType === 'qr'}
						On the device (SeedSigner, Passport, Keystone…): load this vault's registration,
				...
				<p class="khr-copy khr-honest">
					Honest caveat: this proves the key still <strong>derives this vault's addresses</strong>.
			...
			{#if result?.kind === 'ok'}
				<div class="khr-result khr-ok" role="status">
					...
					<span>
						...
						vault expects. Recorded; you're covered for another while.
					</span>
				</div>
				...
				<div class="khr-result khr-warn" role="alert">
					...
					{#if !result.fingerprintMatch}
						<span>
							...
							<span class="mono">{result.deviceFingerprint}</span>, but this vault key was
							...
							vault — treat this key as lost, and consider moving funds while your remaining
