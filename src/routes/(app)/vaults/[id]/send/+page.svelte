<script lang="ts">
	...
	// svelte-ignore state_referenced_locally — per-navigation constant
	const required = data.vault.threshold;
	// svelte-ignore state_referenced_locally — per-navigation constant
	const keys = data.vault.keys;

	// svelte-ignore state_referenced_locally — per-navigation constant
	const resumeTx: SavedVaultTransaction | null = data.resume?.transaction ?? null;
	// svelte-ignore state_referenced_locally — per-navigation constant
	const resumeSummary = data.resume?.summary ?? null;
	// svelte-ignore state_referenced_locally — per-navigation constant
	const resumeProgress: VaultSigningProgress | null = data.resume?.progress ?? null;
	...
	// svelte-ignore state_referenced_locally — intentional per-load seed
	let draft = $state<SavedVaultTransaction | null>(resumeTx);
	...
	// svelte-ignore state_referenced_locally — intentional per-load seed
	let progress = $state<VaultSigningProgress | null>(resumeProgress);

	// What Review/Confirm/Sent render — fresh build or reconstructed on resume.
	type ReviewDisplay = Omit<ConstructedVaultPsbt, 'inputs' | 'change'> & {
	...
	async function build() {
		...
		try {
			const res = await fetch(`/api/vaults/${vaultId}/psbt`, {
				method: 'POST',
			...
			draft = body.draft as SavedVaultTransaction;
			details = body.details as ConstructedVaultPsbt;
			progress = (body.progress as VaultSigningProgress) ?? null;
			activeKeyId = null;
			signFlash = null;
	...
	// signed. The client maps fingerprints onto the vault's key roster to
	...
	type VaultKey = (typeof keys)[number];
	...
	function isSigned(key: VaultKey): boolean {
	...
	function isSpare(key: VaultKey): boolean {
	...
	async function attachSignedPsbt(psbt: string) {
		...
		try {
			const res = await fetch(`/api/vaults/${vaultId}/transactions/${draft.id}`, {
				method: 'PATCH',
				headers: { 'content-type': 'application/json' },
			...
			draft = body.transaction as SavedVaultTransaction;
			const fresh = body.progress as VaultSigningProgress | null;
			if (!fresh) {
	...
	const currentPsbtUrl = $derived(
		draft ? `/api/vaults/${vaultId}/transactions/${draft.id}/file` : '#'
	);
	const registrationUrl = `/api/vaults/${vaultId}/coldcard`;
	...
	async function broadcast() {
		...
		broadcastError = null;
		try {
			const res = await fetch(`/api/vaults/${vaultId}/transactions/${draft.id}/broadcast`, {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({})
			});
...
<div class="send-page" bind:this={pageEl}>
	<header class="page-head">
		<a class="back" href={`/vaults/${vaultId}`}>
			<Icon name="chevron-left" size={15} />
			<span>{data.vault.name}</span>
		</a>
		<h1 class="page-title">Send from vault</h1>
		...
		<section class="step-body fade-in" tabindex="-1" aria-label={stepAriaLabel}>
			<HowItWorks id="vault-send-psbt">
				<p>
					...
					> describing exactly what will be sent. Because this vault is {quorum} multisig, no single
				...
				<p>
					<strong>That's the point of a vault:</strong> a thief (or a bug) with one key gets nothing.
			...
			<div class="card card-pad stack" style="gap: 18px">
				...
				<div class="row" style="justify-content: flex-end; gap: 10px">
					<a class="btn btn-ghost" href={`/vaults/${vaultId}`}>Cancel</a>
				...
				{#key `${activeKey.id}-${signerEpoch}`}
					{#if effectiveDevice === 'qr'}
						...
						<div class="register-callout" role="note">
							...
							<div>
								<strong>First time signing with “{data.vault.name}” on this device? Register the
									vault on it first.</strong>
								SeedSigner, Passport, and Keystone <em>refuse to sign</em> for a multisig vault
								...
								expected, not a bug: register the vault and scan again.
	...
	{:else if step === 'sent'}
		<section class="step-body fade-in sent-body" tabindex="-1" aria-label={stepAriaLabel}>
			...
			<p class="text-secondary">
				Your {quorum} vault transaction is on its way to the network.
			</p>
			...
			<div class="row step-actions" style="justify-content: center">
				<a class="btn btn-secondary" href={`/vaults/${vaultId}`}>Back to vault</a>
				<a class="btn btn-primary" href={`/vaults/${vaultId}/send`} data-sveltekit-reload
					>Send another</a
				>
			</div>
		</section>
...
<style>
	...
	/* ---- registration callout (QR variant; the ColdCard one lives in
	       VaultFileSigner). Warning-toned: registration is a hard prerequisite
