<script lang="ts">
	...
	// Live USB multisig signing for one vault key. A vault-local sibling of the
	...
	// a one-time on-device review of the vault's name, quorum and every
	...
	let {
		...
		unsignedPsbt: string;
		/** Which vault key this signature is being collected from. */
		keyName: string;
		...
		/** The vault's full cosigner roster, in position order. */
		vaultKeys: VaultSignKey[];
	...
	// Stored registrations for this vault; null until the lookup resolves. A
	...
	async function loadRegistrations() {
		try {
			const res = await fetch(`/api/vaults/${vaultId}/ledger-registration`);
	...
	// the key being signed (we know its fingerprint), else — when the vault has
	...
	function fail(err: unknown) {
		if (err instanceof LedgerError) {
			...
			// fingerprints (connected vs the vault's expected set).
			error = err.message;
	...
	async function connectAndSign(forceRegister = false) {
		...
		try {
			if (reg) {
				// Fast path: this device already approved the vault — sign directly.
				phase = 'signing';
				try {
					const signed = await signVaultPsbtWithLedger({
						unsignedPsbt,
						threshold,
						keys: vaultKeys,
						scriptType,
			...
			phase = 'registering';
			const result = await registerVaultPolicy({
			...
			const policyName = sanitizeVaultPolicyName(vaultName);
...
<div class="card card-pad method-active">
	...
	<HowItWorks id="vault-ledger-sign">
		<p>
			...
			<strong>{threshold}-of-{totalKeys} vault</strong>. It returns one more signature, which Cairn
			merges into the transaction.
		</p>
		<p>
			A Ledger co-signs only for vaults it has <strong>registered</strong>: a one-time on-device
			review of the vault's name, its {threshold}-of-{totalKeys} quorum, and every cosigner key.
	...
	{:else}
		{#if needsRegistration}
			...
			<div class="register-callout" role="note">
				...
				<div>
					<strong>First time signing with “{vaultName}” on a Ledger? A one-time registration comes
						first.</strong>
					Your Ledger reviews and stores this vault's details — the name, the
		...
		<div class="verify-callout" role="note">
			...
			<p class="verify-body">
				...
				<strong>{threshold}-of-{totalKeys} vault</strong> and walk through each output. Check the
			...
			<dl class="verify-facts">
				...
				{#if changeSats > 0}
					<div class="fact">
						<dt>Change back to the vault</dt>
		...
		{#if saveWarning}
			<div class="save-warning" role="status">
				...
				<span>
					The Ledger approved the vault, but Cairn couldn't save the registration — signing still
				...
				{:else if offerReregister}
					<div class="reregister-actions">
						<button class="btn btn-secondary btn-sm" onclick={() => connectAndSign(true)}>
							<Icon name="refresh" size={14} /> Vault not recognized on the device? Register it
							again
						</button>
					</div>
				{/if}
		...
		<div class="actions">
			<button class="btn btn-primary" onclick={() => connectAndSign()} disabled={busy}>
				{#if phase === 'registering'}
					<span class="spinner"></span> Approve the vault on your Ledger…
			...
			{#if registrations === null}
				<span class="hint">Checking this vault's Ledger registrations…</span>
			{/if}
		</div>
		...
		{#if phase === 'registering'}
			<p class="hint signing-hint">
				...
				through the registration — it shows “{sanitizeVaultPolicyName(vaultName)}”, the
