<script lang="ts">
	...
	// travels to the device with every request, the Trezor shows the vault's
	...
	async function connectAndSign() {
		...
		signing = true;
		try {
			const signed = await signVaultPsbtWithTrezor({
				unsignedPsbt,
				threshold,
				keys: vaultKeys,
				scriptType
			...
			// signVaultPsbtWithTrezor throws typed, plain-language TrezorErrors —
...
<div class="card card-pad method-active">
	...
	<HowItWorks id="vault-trezor-sign">
		<p>
		...
		<p>
			No registration step is needed: unlike some devices, a Trezor doesn't store the vault — it
	...
	{:else}
		...
		{#if signing}
			<p class="hint signing-hint">
				...
				unlock the device and confirm. The Trezor shows “{vaultName}”'s quorum and each output;
