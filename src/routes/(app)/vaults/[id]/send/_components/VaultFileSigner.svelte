<script lang="ts">
	...
	// The file round trip for one vault key: download the CURRENT combined
	...
	//              the first-time vault-registration callout (an air-gapped
	//              device refuses to co-sign for a vault it has never seen;
	...
	//
	// This is a vault-local sibling of the wallets flow's ColdCardSigner: that
	// component hard-codes the /api/wallets file URL and predates the
	// registration requirement, so vault sends get their own copy of the
	// idiom pointed at the vault endpoints. It stays a pass-through — the
	...
	let {
		...
		fileUrl: string;
		/** The vault registration file (ColdCard format) download. */
		registrationUrl: string;
		vaultName: string;
		threshold: number;
		totalKeys: number;
		/** Which vault key this round trip is collecting a signature from. */
		keyName: string;
...
<div class="card card-pad method-active">
	...
	{#if flavor === 'coldcard'}
		<HowItWorks id="vault-send-coldcard">
			<p>
				...
				{threshold}-of-{totalKeys} spend from “{vaultName}”. Read those off the device before
		...
		<div class="register-callout" role="note">
			...
			<div>
				<strong>First time signing with “{vaultName}” on this ColdCard? Register the vault on the
					device first.</strong>
				A ColdCard <em>refuses to sign</em> multisig transactions for a vault it doesn't know —
...
<style>
	...
	/* ---- first-time vault-registration callout ----
