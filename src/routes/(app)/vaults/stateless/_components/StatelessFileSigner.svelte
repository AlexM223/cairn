<script lang="ts">
	...
	// The file round trip for one stateless-vault key: download the CURRENT
	// combined PSBT, sign it on any PSBT-capable signer, load the result back.
	//
	// A deliberately compact sibling of the vault send flow's VaultFileSigner
	// (duplicated, not imported: that component downloads through server file
	// URLs — /api/vaults/[id]/…/file — which don't exist for a vault that was
	...
	function downloadRegistration() {
		downloadBlob(registration, 'cairn-vault-registration.txt', 'text/plain');
	}
	...
	// normalizes and validates either way. (Same reader as VaultFileSigner.)
...
<div class="card card-pad method-active">
	...
	<div class="register-callout" role="note">
		...
		<div>
			<strong>First time signing for “{vaultLabel}” on this device? It may need the vault
