<script lang="ts">
	...
	// The persistent flow (VaultLedgerSigner) stores that HMAC via
	// /api/vaults/[id]/ledger-registration so later sessions skip straight to
	// signing. A stateless vault has no row to hang the HMAC on, so this
	...
	// who wants the one-time registration wants a persistent vault.
	let {
		unsignedPsbt,
		keyName,
		vaultLabel,
		...
		/** The vault's full cosigner roster, in config order. */
		...
		try {
			...
			if (!reg) {
				...
				const result = await registerVaultPolicy({
					policyName: vaultLabel,
					threshold,
					keys: vaultKeys,
					scriptType
				});
				reg = {
					policyName: sanitizeVaultPolicyName(vaultLabel),
...
<div class="card card-pad method-active">
	...
	<HowItWorks id="stateless-ledger-sign">
		<p>
			...
			<strong>{threshold}-of-{totalKeys} vault</strong> and returns one more signature.
		</p>
		<p>
			A Ledger co-signs only for vaults it has <strong>registered</strong> — an on-device review of
			...
			persistent vault if you want one-time registration.)
		</p>
	</HowItWorks>
	...
	{:else}
		{#if !sessionRegistration}
			<div class="register-callout" role="note">
				...
				<div>
					<strong>Your Ledger will review “{sanitizeVaultPolicyName(vaultLabel)}” before
						signing.</strong>
		...
		{#if phase === 'registering'}
			<p class="hint signing-hint">
				...
				through the registration — it shows “{sanitizeVaultPolicyName(vaultLabel)}”, the
