<script lang="ts">
	...
	// ------------------------------------------------------------- CREATE step
	// Amount stored as canonical `sats` (0 = empty/invalid); AmountEntry owns all
	// unit display/entry so the page never handles BTC/sats/fiat conversion.
	...
	// Fee tier + effective rate are owned by FeeSpeedPicker; the page keeps only
	// the bound values it needs — `feeRate` for build()/canBuild, `feeChoice` for
	// the Review card's arrival words.
	...
	//     Same shape as the single-sig send rail; `spendableSats` is the multisig
	//     balance source. ---
	const summaryAmountSats = $derived(
		review?.amount ?? (isMax ? (spendableSats ?? 0) : rows.reduce((s, r) => s + r.sats, 0))
	);
	...
	const summaryRemainingSats = $derived(
		spendableSats != null
			? Math.max(0, spendableSats - summaryAmountSats - (summaryFeeSats ?? 0))
			: null
	);
	...
	// button now arms a 5s grace window (BroadcastGraceControl, shared with
	// the single-sig send flow) instead of calling broadcast() straight away
	// — an undo window, doctrinally distinct from the removed modal, applied
	// to the friction ladder's one rung with no prior undo. broadcast() is
	// unchanged.
	// svelte-ignore state_referenced_locally — intentional per-load seed
	...
	// Mirrors BroadcastGraceControl's counting state — disables the Confirm
	// step's in-page "Back" button during the window (see the single-sig
	// send page's matching comment).
...
<div class="send-page hw-owns-header" bind:this={pageEl}>
	...
	<!-- Desktop (>=1160px): the wizard step column stays at reading measure with a
	     persistent running-summary rail beside it (docs/DESKTOP-LAYOUT-DESIGN.md §4
	     Send). Below 1160 the rail is display:none and the step column is the single
	     column it always was. Only layout containers are added; the step chain,
	     cosigner/signer components and sticky footers inside `.send-main` are
	     unchanged. -->
	...
	{#if step === 'create'}
		<section class="step-body fade-in" tabindex="-1" aria-label={stepAriaLabel}>
			...
			<FeeSpeedPicker
				fees={chain?.fees ?? null}
				bind:feeRate
				bind:choice={feeChoice}
				loading={chainLoading}
				note={`Multisig inputs are larger than single-signature ones (${quorum} needs ${required} signatures per coin), so the same fee rate costs a little more in total fees.`}
			/>
				...
				{#snippet detailExtra()}
					{#if review}
						...
						{#if review.inputs.length > 0}
							<div class="detail-row coins-head">
								<span class="text-secondary"
...
<style>
	...
	@media (min-width: 1160px) {
		/* A reading page with a rail needs room for the ~720px step column plus the
		   280px rail; widen <main> (only for the send page) to step + gutter + rail
		   so the step column stays at reading measure and the surplus is the rail. */
