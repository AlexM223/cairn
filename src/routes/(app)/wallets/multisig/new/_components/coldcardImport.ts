...
import type { MultisigScriptType } from '$lib/server/wallets/multisig';
...
export function parseColdcardExport(text: string, scriptType: MultisigScriptType): ColdcardKey {
	let doc: unknown;
	try {
		doc = JSON.parse(text);
	} catch {
