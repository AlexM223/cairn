/**
 * Stratum V1 TCP server for the Heartwood SOLO mining engine.
 *
 * Near-verbatim port of the Tessera pool Stratum server
 * (C:\dev\raffle\pool\src\stratum.ts), with the raffle finder/registry machinery
 * removed and multi-user solo authorization added. Retained hardening:
 ...
 *  - JOB_WINDOW = 4 validity window; (en1:en2:nonce) duplicate rejection; strict
 *    hex validation; ntime must equal the job's; error codes 20–25.
 *  - Per-connection vardiff (×2 / ÷2, power-of-two snap, clamp
 ...
 *  - Loopback-only bind default; maxConnections default 64.
 *
 * SOLO CHANGES vs Tessera:
 ...
 *      escape and crash the in-process web server.
 *
...
export interface VardiffOptions {
	...
	readonly now?: () => number;
	/** Hard ceiling on per-connection difficulty (overflow-DoS guard). Default 2^48. */
...
export interface StratumServerOptions {
	...
	/** REGTEST_POLICY_SHIFT; 0 disables (production semantics). Default 4. */
...
function nearestPowerOfTwo(d: number): number {
...
export class StratumServer {
	...
	constructor(opts: StratumServerOptions) {
		...
		if (v === undefined) {
			...
			if (maxDifficulty < opts.shareDifficulty) {
				throw new Error(
					`vardiff.maxDifficulty (${maxDifficulty}) must be >= shareDifficulty (${opts.shareDifficulty})`
				);
			}
			this.vd = {
	...
	private onConnection(socket: Socket): void {
		...
		const onData = makeLineSplitter(
			(line) => {
				...
				// and crash the whole web server. Catch, log, and drop the connection.
				try {
	...
	private sendDifficultyAndJob(conn: ConnState, built: BuiltJob | null): void {
		...
		// at actually CHANGES (see the Tessera port comment): only the FIRST
		// announce of a genuinely new difficulty re-bases.
	...
	private handleSubmit(conn: ConnState, id: MsgId, params: unknown[]): void {
		...
		if (ntime !== built.job.ntimeHex.toLowerCase()) {
			// PoC strictness: ntime rolling is not allowed.
		...
		if (hashValue > target) {
			// Reject WITHOUT recording the dedup key (see the Tessera port note):
			// only ACCEPTED shares are deduped, bounding set growth by real hashrate.
	...
	private recordAcceptedShare(conn: ConnState): void {
		...
		const ratePerMin = (conn.shareTimes.length * 60_000) / observeMs;
		let next: number;
		if (ratePerMin > vd.targetSharesPerMin * (1 + VARDIFF_RATE_TOLERANCE)) next = conn.difficulty * 2;
		else if (ratePerMin < vd.targetSharesPerMin * (1 - VARDIFF_RATE_TOLERANCE)) next = conn.difficulty / 2;
		else return;
		// Ceiling clamp BEFORE the snap so nearestPowerOfTwo never sees a runaway
		// or Infinity value (an uncaught throw there would crash the process).
		next = Math.min(next, vd.maxDifficulty);
		next = nearestPowerOfTwo(next);
		if (next < this.opts.shareDifficulty) next = this.opts.shareDifficulty; // floor clamp AFTER snap
		if (next === conn.difficulty) return; // pinned at the floor — no-op
