/**
 * Per-user mining preferences (mining_prefs table): the miner's stable Stratum
 * identity token, whether they've opted in, and which of their wallets receives
 * the full reward when they find a block.
 *
...
import { onPrefsChanged } from './index';

const log = childLogger('mining:prefs');

export interface MiningPrefs {
	userId: number;
	/** Stratum username token: `hw_` + 8 lowercase hex. Null only transiently
	 *  before ensureMiningPrefs has generated one. */
...
/** `hw_` + 8 lowercase hex characters (4 random bytes). */
function generateMiningId(): string {
	return `hw_${randomBytes(4).toString('hex')}`;
}

function rowFor(userId: number): PrefsRow | undefined {
	return db.prepare('SELECT * FROM mining_prefs WHERE user_id = ?').get(userId) as
		| PrefsRow
		| undefined;
}
 ...
 * touch. The mining_id is generated with a uniqueness retry: a collision on the
 * UNIQUE(mining_id) index (astronomically unlikely at 2^32 space) just draws a
 * new id. Idempotent — an existing row with a mining_id is returned unchanged.
 */
export function ensureMiningPrefs(userId: number): MiningPrefs {
	...
	// Row absent, or present but somehow missing a mining_id — assign one.
	for (let attempt = 0; attempt < 8; attempt++) {
		...
		try {
			...
			log.warn({ err: e, userId, attempt }, 'mining_id collision, retrying');
		}
	}
...
/**
 * Point the user's payout at one of THEIR wallets, or null to clear it. Rejects
 * a wallet that isn't the caller's own or that can't receive (no xpub) — a
 * foreign or unpayable wallet must never be settable as a payout target, since
 * the engine would otherwise try to build a coinbase to an address the user
 * doesn't control. Ensures the prefs row exists first.
 */
export function setPayoutWallet(userId: number, walletId: number | null): MiningPrefs {
	...
	if (walletId !== null) {
		...
		if (!wallet.xpub || wallet.xpub.trim() === '')
 ...
 * still authorizing with it is rejected UNAUTHORIZED and must be reconfigured.
 */
export function regenerateMiningId(userId: number): MiningPrefs {
	...
	for (let attempt = 0; attempt < 8; attempt++) {
		...
		try {
			...
			log.warn({ err: e, userId, attempt }, 'mining_id collision on regenerate, retrying');
		}
	}
	throw new Error('could not allocate a unique mining_id');
}
