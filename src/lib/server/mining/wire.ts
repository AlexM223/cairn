 ...
 * Stratum server, and block assembler. EVERY byte-order decision lives here and
 * nowhere else — the server and the miner must import these, never reimplement
 * them, so the two sides cannot disagree.
 *
 * PORTED VERBATIM from the Tessera pool (C:\dev\raffle\pool\src\wire.ts): this is
 * consensus-critical byte math and is never re-derived. Only this header comment
...
/**
 * Share weight at a given pool difficulty (weight = share difficulty, as an
 * integer). Fractional pool difficulties are scaled by a constant; the scale
 * cancels in any uniform accounting.
 */
 ...
 * (cap every connection buffer; kill on overflow).
 */
