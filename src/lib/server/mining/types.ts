/**
 * Shared types for the Heartwood SOLO mining engine (src/lib/server/mining).
 *
 * Authored and committed FIRST: this module defines the contract the rest of the
 * engine (job/stratum/tipPoller/miningPool) and the later Heartwood integration
 * wave build against. It is deliberately dependency-light — only a type-only
 * import of bitcoinjs-lib (for the Network shape) is pulled in, so it type-checks
 * standalone.
 *
 * SOLO POOL INVARIANT (legal hard gate cairn-vn43.14): every job's coinbase pays
 * exactly ONE value-bearing output — the authorized miner's own payout script,
 * carrying the FULL template coinbase value — plus the zero-value SegWit
 * witness-commitment output. There is NO reward splitting of any kind: the block
 * finder keeps the entire reward. Any pooled/shared-reward mode is a hard-gated
 * legal decision, never a code change.
 */
 ...
 * Heartwood user, the wallet that receives the block reward, and that wallet's
 * dedicated payout address (pre-encoded to an output script so the hot path
 * never re-derives it). The coinbase for this miner's jobs pays `payoutScript`
 * the FULL coinbase value and nothing else.
 */
...
/**
 * Synchronous, zero-I/O resolver from a Stratum miningId to its MinerAuth. The
 * real implementation (a later wave) is an in-memory snapshot the integration
 * layer refreshes out-of-band; the engine calls `resolve` inside the socket data
 * handler and MUST NOT block on I/O there. Returns null for an unknown/revoked
 * miningId (the connection is then rejected UNAUTHORIZED).
 */
...
/**
 * Trivial map-backed AuthProvider — the test/bootstrap implementation. The real
 * snapshot-backed provider ships in the integration wave; this satisfies the
 * synchronous zero-I/O contract for unit tests and standalone bring-up.
 */
 ...
 * fairness rule). A stats bridge sums this per worker for hashrate accounting.
 */
 ...
 * everything the coordinator needs to re-personalize the exact winning coinbase
 * (frozen `payoutScriptHex`, never the miner's live address after a wallet
 * change) and to attribute + notify the win.
 */
...
// ---------------------------------------------------------------------------
// Status projection (what the stats/control API surfaces)
// ---------------------------------------------------------------------------
...
/**
 * One mining job's SHARED fields (all hex per src/lib/server/mining/wire.ts
 * conventions). There is no base coinb1/coinb2 here: a solo coinbase has no
 * meaning without a payout script, so every connection is announced its OWN
 * personalized coinb1/coinb2 (see BuiltJob.personalize) over these shared
 * fields and the shared merkle branches.
 */
